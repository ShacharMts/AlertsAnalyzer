...
import { getAlerts, refreshCityAlerts } from "@/lib/orefClient";
...
export async function POST(request: Request) {
  ...
  try {
    // Fetch fresh data from oref + merge with cache, all in one invocation
    const alerts = await getAlerts(15, true);
    const summary = summarize(alerts, range);
    ...
    return NextResponse.json({ summary, timeseries, cities, cityTimeseries });
  } catch (err) {
