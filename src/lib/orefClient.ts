...
let lastOrefStatus: number | undefined;
let lastOrefSample: OrefAlert[] = [];
let lastOrefUsedFallback = false;
...
async function fetchLatestAlerts(): Promise<OrefAlert[]> {
  ...
  if (!text || text.trim() === "") { lastOrefSample = []; return []; }
  const parsed = JSON.parse(text) as OrefAlert[];
  lastOrefSample = parsed.slice(0, 3);
  return parsed;
}
...
async function fetchCityAlertsFromOref(hebrewName: string): Promise<OrefAlert[]> {
  ...
  if (res.status === 403) {
    // mode=3 blocked (geo/IP restriction on Netlify) — fallback to mode=0 and filter
    console.warn(`[oref] mode=3 returned 403 for ${hebrewName}, falling back to mode=0`);
    lastOrefUsedFallback = true;
    const allAlerts = await fetchLatestAlerts();
    const filtered = allAlerts.filter((a) => a.data.includes(hebrewName));
    console.log(`[oref] Fallback: ${allAlerts.length} total → ${filtered.length} matching "${hebrewName}"`);
    lastOrefSample = filtered.slice(0, 3);
    return filtered;
  }
...
export interface RefreshMeta {
  ...
  totalRecords: number;
  source: string;
  orefResponseSample?: unknown[];
}

// Last refresh metadata for retrieval after getAlerts call
let lastRefreshMeta: RefreshMeta | null = null;
export function getLastRefreshMeta(): RefreshMeta | null { return lastRefreshMeta; }
...
export async function getAlerts(daysBack: number = 15, forceRefresh: boolean = false): Promise<Alert[]> {
  ...
  if (!forceRefresh && cachedAlerts && now - cacheTimestamp < CACHE_TTL_MS) {
    lastRefreshMeta = { orefCalled: false, freshRecords: 0, cachedRecords: cachedAlerts.length, totalRecords: cachedAlerts.length, source: "memory-cache" };
    return cachedAlerts;
  }

  // On serverless (Netlify/Vercel), oref blocks requests (403). Skip and use bundled cache.
  if (isServerless) {
    const diskData = loadDiskCache();
    const historical = generateHistorical(diskData, daysBack);
    const allData = [...diskData, ...historical];
    cachedAlerts = mapOrefToAlerts(allData);
    cacheTimestamp = now;
    lastRefreshMeta = { orefCalled: false, freshRecords: 0, cachedRecords: diskData.length, totalRecords: cachedAlerts.length, source: "bundled-cache" };
    console.log(`[oref] Serverless: serving ${diskData.length} from bundled cache, ${historical.length} synthetic`);
    return cachedAlerts;
  }

  const orefUrl = "https://alerts-history.oref.org.il/Shared/Ajax/GetAlarmsHistory.aspx?lang=he&mode=0";
  ...
  try {
    ...
    console.log(
      `[oref] Fetched ${freshData.length} fresh from ${orefUrl}, disk cache ${merged.length}, synthetic ${historical.length}, covering ${distinctDays.size} days`
    );

    cachedAlerts = mapOrefToAlerts(allData);
    cacheTimestamp = now;
    lastRefreshMeta = { orefCalled: true, orefUrl, orefStatus: lastOrefStatus, freshRecords: freshData.length, cachedRecords: existing.length, totalRecords: cachedAlerts.length, source: "oref-api", orefResponseSample: lastOrefSample };
    return cachedAlerts;
  } catch (err) {
    console.error("Failed to fetch oref data:", err);
    lastRefreshMeta = { orefCalled: true, orefUrl, orefStatus: lastOrefStatus, freshRecords: 0, cachedRecords: 0, totalRecords: cachedAlerts?.length || 0, source: "error" };
...
export async function refreshCityAlerts(cityId: string, force: boolean = false): Promise<{ refreshed: boolean; count: number }> {
  ...
  if (!force && now - lastFetch < CITY_CACHE_TTL_MS) {
    lastRefreshMeta = { orefCalled: false, freshRecords: 0, cachedRecords: 0, totalRecords: cachedAlerts?.length || 0, source: "city-cache-ttl" };
  ...
  if (isServerless) {
    const existing = loadDiskCache();
    lastRefreshMeta = { orefCalled: false, freshRecords: 0, cachedRecords: existing.length, totalRecords: existing.length, source: "bundled-cache" };
    cityFetchTimestamps.set(cityId, now);
    return { refreshed: false, count: 0 };
  }

  const orefUrl = `https://alerts-history.oref.org.il/Shared/Ajax/GetAlarmsHistory.aspx?lang=he&mode=3&city_0=${encodeURIComponent(city.he)}`;

  try {
    const freshData = await fetchCityAlertsFromOref(city.he);
    const actualUrl = lastOrefUsedFallback
      ? "https://alerts-history.oref.org.il/Shared/Ajax/GetAlarmsHistory.aspx?lang=he&mode=0 (fallback: mode=3 got 403, used mode=0 filtered for " + city.he + ")"
      : orefUrl;
    if (freshData.length > 0) {
      ...
      lastRefreshMeta = { orefCalled: true, orefUrl: actualUrl, orefStatus: lastOrefStatus, freshRecords: freshData.length, cachedRecords: existing.length, totalRecords: merged.length, source: lastOrefUsedFallback ? "oref-mode0-fallback" : "oref-city-api", orefResponseSample: lastOrefSample };
    } else {
      const existing = loadDiskCache();
      lastRefreshMeta = { orefCalled: true, orefUrl: actualUrl, orefStatus: lastOrefStatus, freshRecords: 0, cachedRecords: existing.length, totalRecords: existing.length, source: "oref-city-api-empty", orefResponseSample: [] };
    }
    ...
    lastRefreshMeta = { orefCalled: true, orefUrl, orefStatus: lastOrefStatus, freshRecords: 0, cachedRecords: 0, totalRecords: 0, source: "error" };
    return { refreshed: false, count: 0 };
  }
}
