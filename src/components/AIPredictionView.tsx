...
import { AIPrediction, HourlyRisk } from "@/lib/aiPredictor";
...
export default function AIPredictionView({ gpsCityId, selectedCityId, onCityChange }: Props) {
  ...
  const fetchPrediction = useCallback(async (cityId: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(withDevice(`/api/alerts?action=ai-predict&cityId=${encodeURIComponent(cityId)}`));
  ...
  const riskLabel = (level: string) => {
    const labels = t.ai?.riskLevels || { low: "Low", medium: "Medium", high: "High", critical: "Critical" };
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        <div className="flex flex-wrap items-center gap-4">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
            {t.ai?.selectCity || t.prediction.selectCity}
          </label>
          <select
            value={selectedCity}
          ...
          >
            <option value="">{t.ai?.selectCityPrompt || "Select a city"}</option>
      ...
      {!selectedCity && (
        <div className="rounded-xl border border-gray-200 bg-white p-8 text-center dark:border-gray-700 dark:bg-gray-900">
          <p className="text-gray-500 dark:text-gray-400">
            {t.ai?.selectCityPrompt || "Select a city to see AI predictions"}
          </p>
        </div>
      )}
      ...
      {loading && selectedCity && (
        <div className="rounded-xl border border-gray-200 bg-white p-8 text-center dark:border-gray-700 dark:bg-gray-900">
          ...
          <p className="mt-2 text-sm text-gray-500">{t.ai?.analyzing || "Analyzing attack patterns..."}</p>
        </div>
      )}
      ...
      {prediction && !loading && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
              <p className="text-xs font-medium text-gray-500 dark:text-gray-400">
                {t.ai?.cityAttacks || "City Attacks (30d)"}
              </p>
              <p className="mt-1 text-2xl font-bold text-red-600">
                {prediction.totalCityAttacks30d}
              </p>
            </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
              <p className="text-xs font-medium text-gray-500 dark:text-gray-400">
                {t.ai?.nearbyAttacks || "Nearby Attacks (30d)"}
              </p>
              <p className="mt-1 text-2xl font-bold text-orange-500">
                {prediction.totalNearbyAttacks30d}
              </p>
              <p className="text-xs text-gray-500">
                {prediction.nearbyCityIds.length} {t.ai?.citiesRadius || "cities within 25km"}
              </p>
            </div>
          </div>

          {/* 24h Risk Timeline Bar Chart */}
          <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
            <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">
              {t.ai?.timeline || "24-Hour Risk Timeline"}
            </h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={prediction.next24h.map((h) => {
                  const dt = new Date(Date.now() + h.hourOffset * 60 * 60 * 1000);
                  return { ...h, chartLabel: `${dt.getDate().toString().padStart(2, "0")}/${(dt.getMonth() + 1).toString().padStart(2, "0")} ${h.label}` };
                })}
              ...
              >
                ...
                <YAxis
                  domain={[0, 100]}
                  tick={{ fontSize: 11 }}
                  label={{ value: t.ai?.riskPct || "Risk %", angle: -90, position: "insideLeft", fontSize: 12 }}
                />
                <Tooltip
                  content={({ active, payload }) => {
                    if (!active || !payload?.length) return null;
                    const d = payload[0].payload as HourlyRisk;
                    return (
                      <div className="rounded-lg border bg-white p-3 text-sm shadow-lg dark:border-gray-600 dark:bg-gray-800">
                        ...
                        <p style={{ color: RISK_COLORS[d.riskLevel] }}>
                          {t.ai?.risk || "Risk"}: {d.riskPct}% ({riskLabel(d.riskLevel)})
                        </p>
                        <p>{t.ai?.avgMissiles || "Avg missiles"}: {d.avgMissiles}</p>
                        <p>{t.ai?.nearbyAvg || "Nearby avg"}: {d.nearbyAvgMissiles}</p>
                        <p>{t.ai?.daysHit || "Days with attacks"}: {d.daysWithAttacks}/{d.daysAnalyzed}</p>
                        <p>{t.ai?.nearbyCities || "Nearby cities hit"}: {d.nearbyCitiesHit}</p>
                      </div>
                    );
                  }}
                />
                ...
                <Bar dataKey="riskPct" radius={[4, 4, 0, 0]}>
                  {prediction.next24h.map((entry, idx) => (
                    <Cell
                      ...
                      stroke={entry.hourOffset === 0 ? "#1d4ed8" : undefined}
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
            <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">
              {t.ai?.hourlyBreakdown || "Hourly Breakdown"}
            </h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-start text-xs font-medium text-gray-500 dark:border-gray-700 dark:text-gray-400">
                    <th className="px-2 py-2">{t.ai?.hour || "Hour"}</th>
                    <th className="px-2 py-2">{t.ai?.risk || "Risk"}</th>
                    <th className="px-2 py-2">{t.ai?.level || "Level"}</th>
                    <th className="px-2 py-2">{t.ai?.avgMissiles || "Avg Missiles"}</th>
                    <th className="px-2 py-2">{t.ai?.nearbyAvg || "Nearby Avg"}</th>
                    <th className="px-2 py-2">{t.ai?.daysHit || "Days Hit"}</th>
                  </tr>
                </thead>
                <tbody>
                  {prediction.next24h.map((h) => {
                    ...
                    <tr
                      ...
                      className={`border-b dark:border-gray-700 ${
                        h.hourOffset === 0 ? "bg-blue-50 dark:bg-blue-900/20" : ""
                      }`}
                    >
                        ...
                        <div className="flex items-center gap-2">
                          <div className="h-2 w-16 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                            <div
                              ...
                              style={{
                                width: `${h.riskPct}%`,
                                background: RISK_COLORS[h.riskLevel],
                              }}
                            ></div>
                          </div>
                      ...
                      <td className="px-2 py-1.5">{h.avgMissiles}</td>
                      <td className="px-2 py-1.5">{h.nearbyAvgMissiles}</td>
                      <td className="px-2 py-1.5">{h.daysWithAttacks}/{h.daysAnalyzed}</td>
                    </tr>
                    );
                  })}
                </tbody>
          ...
          {prediction.nearbyCityIds.length > 0 && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
              <h3 className="mb-2 text-sm font-semibold text-gray-900 dark:text-gray-100">
                {t.ai?.nearbyCitiesTitle || "Nearby Cities Analyzed"} ({prediction.nearbyCityIds.length})
              </h3>
          ...
          <p className="text-center text-xs text-gray-400 dark:text-gray-500">
            {t.ai?.disclaimer || "Predictions are based on historical patterns and do not guarantee future events. Always follow official emergency instructions."}
          </p>
        </>
      )}
    </div>
